...
def set_status(percent=100):
    ...
    if percent > 110.2:
        status = f'''Норма сильно превышена. Нужно уменьшить потребление витамина К на {round(percent - 110.2, 2)}%'''
    elif percent > 106.8:
        status = 'Количистов витамина К находится в максимальной граници нормы'
    elif percent > 103.4:
        status = 'Содержание витамина К близко к идеальному'
    elif percent < 89.8:
        status = f'''Витамин К намного меньше нормы. Необходимо увеличить потребление витамина К на {round(89.8 - percent, 2)}%'''
    elif percent < 93.2:
