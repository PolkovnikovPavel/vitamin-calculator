...
def set_status(percent=100):
    ...
    if percent > 117.2:
        status = f'''Норма сильно превышена, опасно тем, что кровь будет очень густой, черевато тромбозами!
Нужно уменьшить потребление витамина К на {int(percent + 0.5) - 117}%'''
        ...
        status = f'''Витамин К намного меньше нормы, кровь очень жидкая, крайне опасно внутреними и другими кровотечениями!
Необходимо увеличить потребление витамина К на {82.8 - int(percent  + 0.5)}'''
        ...
        status = 'Идиальное содержание витамина К'
    return status
