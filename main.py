...
@login_required   # удаление конкретного рассписания
...
def password_change():   # на будующее, сделать смену пароля
    return render_template("in_developing.html")


@app.route('/timetable_duplicate/<int:id>')
@login_required   # полностью скопировать конкретное рассписание на сегодня
...
@login_required   # для просмотра конкретного рассписания
def look_timetable(id):
    ...
    if request.method == "GET":
        ...
        percents_breakfast = []   # для отображения процента от нормы
        percents_dinner = []
        percents_supper = []
        ...
        values = [result_breakfast, result_dinner, result_supper,
                  len(result_breakfast), len(result_dinner),
                  len(result_supper),
        ...
        return redirect('/')   # происходит по нажатии на кнопку "Вернуться"


@app.route('/timetable/<int:id>', methods=['GET', 'POST'])
@login_required   # изменение существ. рассписания
def edit_timetable(id):
    ...
    if request.method == "GET":
        ...
        return render_template("timetable.html", values=values,
                               products=products, title='Внесение продуктов',
        ...
        return render_template("timetable.html", values=values,
                               title='Внесение продуктов', date=date,
                               products=products, result=result)


@app.route('/timetable')   # для создания нового рассписания
@login_required   # после создания перенапровляет на изменение данного распис.
def add_timetable():
    if not current_user.is_authenticated:
        return redirect('/')   # возврат на главное меню
...
def timetable_without_regist():   # расчёт без регистрации
    ...
    if request.method == "GET":   # всё аналогично
        ...
        return render_template("timetable_without_regist.html", values=values,
                               products=products, title='Подсчёт витамина К',
                               result=result)
        ...
        return render_template("timetable_without_regist.html", values=values,
                               title='Подсчёт витамина К', products=products,
                               result=result)
...
@app.route("/monitoring")   # для мониторинга сайта
def monitoring():
    return render_template("in_developing.html")


@app.route('/logout')   # разлогинивание
@login_required
def logout():
    activity(f'Sign Out')
    logout_user()
